...
public class UIDragObject : MonoBehaviour
{
    ...
    /// </summary>
    public Vector3 min = new Vector3(-1280f, -720f, 0);
    public Vector3 max = new Vector3(1280f, 720f, 0);

    void Move (Vector3 worldDelta)
	{
        if(
            target.localPosition.x < min.x && worldDelta.x < 0 ||
            target.localPosition.y < min.y && worldDelta.y < 0 ||
            target.localPosition.z < min.z && worldDelta.z < 0 ||
            target.localPosition.x > max.x && worldDelta.x > 0 ||
            target.localPosition.y > max.y && worldDelta.y > 0 ||
            target.localPosition.z > max.z && worldDelta.z > 0
         )
        {
            //CancelMovement();
            return;
